"""
Buriti Alegre Ambiental — Dashboard BI
...
# ── Config ────────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Buriti Alegre Ambiental | BI",
    page_icon="🌿",
    layout="wide",
    ...
    <thead>
      <tr style="background:#fafcff;">
        ...
        <th style="padding:7px 12px;text-align:right;font-size:.72rem;font-weight:700;
                   text-transform:uppercase;letter-spacing:.5px;color:#2E7D32;">◉ {lbl_atual}</th>
        <th style="padding:7px 12px;text-align:right;font-size:.72rem;font-weight:700;
...
def pg_cockpit(D, d0, d1):
    ...
    if not fat.empty and "nr_economia_agua" in fat.columns:
        ...
        if _comp and not agg.empty:
            ...
            series_fm = [
                ("Faturamento Total",       (agg["vl_agua"] + agg["vl_tbas_a"]) / agg["nr_eco"], "#2E7D32", "top center", 4),
...
def pg_cortes(D, d0, d1):
    ...
    if "nm_bairro_dim" in cor.columns:
        ...
        fig3.update_traces(
            ...
            insidetextanchor="middle",
            textangle=0,
            textfont=dict(family="Arial Black", color="white", size=15),
            cliponaxis=False,
        )
        fig3.update_layout(margin=dict(t=35, b=0, l=0, r=20), xaxis_title="", yaxis_title="")
        st.plotly_chart(fig3, width="stretch")
...
def pg_frota_combustivel(D, d0, d1):
    ...
    if frota.empty:
        st.info("📊 Dados de Frota e Combustível não disponíveis ainda. Alimentação prevista após integração com BigQuery.")
        return
...
def pg_energia(D, d0, d1):
    ...
    if ene.empty:
        st.info("📊 Dados de Energia Elétrica não disponíveis ainda. Alimentação prevista após integração com BigQuery.")
        return
...
def pg_tratamento(D, d0, d1):
    ...
    if prod.empty and qual.empty:
        st.info("📊 Dados de Tratamento não disponíveis ainda. Alimentação prevista após integração com BigQuery.")
        return
...
def main():
    ...
    if st.sidebar.button("📊 Gerar Apresentação PPTX", use_container_width=True,
        ...
        with st.sidebar:
            with st.spinner("Gerando apresentação..."):
                try:
                    import pptx_report
                    pptx_bytes = pptx_report.gerar_pptx(D, d0, d1, pptx_report.CFG_BURITI)
    ...
    if st.session_state.get("pptx_bytes"):
        ...
        _nome = f"BI_BuritiAlegre_{pd.Timestamp(_d0p).strftime('%Y%m')}_{pd.Timestamp(_d1p).strftime('%Y%m')}.pptx"
